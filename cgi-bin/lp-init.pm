...
my $cci_id = $main::session{cci_id}||$cgi->param('cci_id'); #print "Session : $cci_id";
my ($session_id,$emplid) = CCICryptography::getEmpid($cci_id); 
#print "$session_id,$emplid";
...
if ( $main::cgi{server_name} eq 'cta.ccionline.biz' ) {
	...
	$main::session{home_url} = "http://centurylinkyoucandev.com/cta-index.cgi";
}
