...
# 
#	expects staff_id as parameter; staff_id of user
# 	vccust12 is the SSN (and it's encrypted)
########################################################
...
my $staff_id = $cgi->param('staff_id');
...
my $sql;
my (%data, $cust_id, $encrypteddata, $contact_main_id, $contact_personnel_id, $msg);

if ( $cgi->param('savedata') ne "" ) {
    # Ok, we know the staff_id; so go get the cust_id
    ...
    $sql = "select count(contact_main_id) as cnt from contact_main with (nolock)
            where cust_id = ?";
			
	my $sth = $myDB->prepare($sql);
	$sth->execute($cust_id);
	my $cntdata = $sth->fetchrow_hashref();
	$sth->finish();

    my $thecnt = $cntdata->{cnt};

    # check to see if they already have a record.  Why it would do this twice, I dunno, but just in case
    if ( $qwest == 1 ) {

        
		$sql = "select top 1 contact_main_id from contact_main with (nolock) where cust_id = ? order by 1 desc";
        ...
        $contact_main_id = $data1->{contact_main_id};
		
		# check if they have contact_personnel record
        # 
		$sql = "select 0 as contact_personnel_id union select contact_personnel_id from contact_personnel with (nolock) where contact_main_id =  $contact_main_id order by 1 desc ";
        ...
        $contact_personnel_id = $data1->{contact_personnel_id};
